...
ücret:<input type="text" name="ücret" value="10000"><br>
oda Sayisi:<?php
$sql = "SELECT distinct(oSayisi) as a FROM  ev";
$result = $conn->query($sql);
echo '<select name="oSayisi">';
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {  
		echo	"<option selected value=".$row["a"].">".$row["a"]."</option>";
    }
} 
    echo "</select><br>";
	?><br>
...
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {  
		echo	"<option selected value=".$row["eNo"].">".$row["eTip"]."</option>";
    }
} 
    echo "</select><br>";
	?>
...
<h2>Kiracıyı ilişkilendirin</h2>
<div>

<form action="addKira.php" method="post">
<?php 	
if(isset($_GET['ücret'])&&isset($_GET['oSayisi'])&& isset($_GET['tip']) && isset($_GET['adres']))
...
bitis tarih:<input type="text" name="bitis" class="datepicker" /><br>
	<p id="calis" ></p><?php
echo "kiracıyı ekleyiniz....";
